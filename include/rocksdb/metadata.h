...
struct SstFileMetaData : public FileStorageInfo {
  ...
  SstFileMetaData(const std::string& _file_name, uint64_t _file_number,
                  ...
                  size_t _blob_file_set_count, uint64_t _oldest_ancester_time,
      ...
      : smallest_seqno(_smallest_seqno),
        ...
        blob_file_set_count(_blob_file_set_count),
  ...
  // Size of the complete set of blob files referenced by this SST.
  // Only available when the sst file is created after
  // enable_blob_file_set_record is on.
  size_t blob_file_set_count = 0;
...
struct BlobMetaData {
  ...
  BlobMetaData(uint64_t _file_number, const std::string& _file_name,
               ...
               uint64_t _linked_ssts_count = 0,
               uint64_t _full_linked_ssts_count = 0)
      : blob_file_number(_file_number),
        ...
        linked_ssts_count(_linked_ssts_count),
        full_linked_ssts_count(_full_linked_ssts_count) {}
  ...
  // Number of SST files that reference this blob file through their
  // oldest_blob_file_number.
  uint64_t linked_ssts_count;
  // Number of SST files that reference this blob file through their complete
  // blob_file_set record.
  // Only available when the blob is created after enable_blob_file_set_record
  // is on.
  uint64_t full_linked_ssts_count;
};
